...
/// Returns [Some] number of steps (length - 1) of the
/// [Collatz
/// Sequence](https://en.wikipedia.org/wiki/Collatz_conjecture)
/// starting at `x`; if `x` is 0 or the calculation
/// overflows, instead return [None].
///
/// # Examples
///
/// ```
...
pub fn collatz_length(mut x: u64) -> Option<usize> {
    ...
    while x != 1 {
        match x & 1 == 1 {
            true => {
                // x is odd.
                x = x.checked_mul(3)?.checked_add(1)?;
            }
            false => {
                // x is even.
                x /= 2;
...
mod test {
    ...
    fn test_collatz_length() {
        assert!(collatz_length(0).is_none());
        test_1(1, 0);
        ...
        // Overflows.
        assert!(collatz_length(0xffff_ffff_ffff_ffff).is_none());
    }
}
